...
InputParameters validParams<SolidModel>()
{
  ...
  params.addParam<int>("compute_method", 1, "The method  used in the stress calculation.");
...
SolidModel::SolidModel( const InputParameters & parameters) :
  ...
  _compute_method( getParam<int>("compute_method") ),
...
SolidModel::computeProperties()
{
  ...
  for ( _qp = 0; _qp < _qrule->n_points(); ++_qp )
  {
    ...
    if(_compute_method >= 4)
      {
        // SymmTensor tensor;
  //        const ColumnMajorMatrix & R( (*_crack_rotation_old)[_qp] );
  //        tensor = _stress_old;
  ...
  //  SymmIsotropicElasticityTensor * t = dynamic_cast<SymmIsotropicElasticityTensor*> ( &(_elasticity_tensor_old[_qp]) );

  //     _local_elasticity_tensor->form9x9Rotation(R, R_9x9);
  //  //     _local_elasticity_tensor->rotateFromGlobalToLocal(R_9x9);
  //  _local_elasticity_tensor->adjustForCracking((*_crack_flags)[_qp], true);
  _stress[_qp] = (*_local_elasticity_tensor) * ePrime;
  //        applyCracksToTensor( _stress[_qp], sigma );
  rotateSymmetricTensor(R, _stress[_qp], _stress[_qp] );
...
SolidModel::crackingStrainDirections()
{
  ...
  if (cracking_locally_active)
  {
    ...
    // constitutive law.
    if(_compute_method >= 2)
      _local_elasticity_tensor->adjustForCracking( _crack_flags_local, true);
    else
      _local_elasticity_tensor->adjustForCracking( _crack_flags_local, false);
